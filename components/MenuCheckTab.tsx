...
const ResultItemCard: React.FC<{ item: MenuCheckResultItem }> = ({ item }) => {
    const statusStyles = {
...
const MenuCheckTab: React.FC<TabProps> = ({ addLog }) => {
        ...
        <section id="menu">
            ...
            <div className="space-y-4 mb-6">
                <div>
                    ...
                    <input type="url" id="web-menu-url" value={webMenuUrl} onChange={(e) => setWebMenuUrl(e.target.value)} placeholder="e.g., https://www.store.com/menu" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border" />
                </div>
                <div>
            ...
            {results && (
                <div className="mt-8 border-t pt-6">
                    ...
                    <div className="space-y-4">
                        {results.map((item, index) => <ResultItemCard key={index} item={item} />)}
                    </div>
                </div>
            )}
        </section>
