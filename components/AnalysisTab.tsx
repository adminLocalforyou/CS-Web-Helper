...
const AnalysisTab: React.FC<TabProps> = ({ addLog }) => {
    ...
    const renderQualitativeAssessment = (assessment: string) => {
        const parts = assessment.split('---TH---');
        const englishPart = parts[0]?.trim();
        const thaiPart = parts[1]?.trim();
        ...
        <section id="analysis">
            ...
            <div className="space-y-4 mb-6">
                <div>
                    ...
                    <input type="url" id="website-url" value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} placeholder="e.g., https://www.restaurant-name.com" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
                    <label htmlFor="gmb-url" className="block text-sm font-medium text-gray-700">Google My Business URL</label>
                    <input type="url" id="gmb-url" value={gmbUrl} onChange={(e) => setGmbUrl(e.target.value)} placeholder="e.g., https://g.page/restaurant-gmb-link (AI will check reviews from here)" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
                    <label htmlFor="facebook-url" className="block text-sm font-medium text-gray-700">Facebook Page URL</label>
                    <input type="url" id="facebook-url" value={facebookUrl} onChange={(e) => setFacebookUrl(e.target.value)} placeholder="e.g., https://www.facebook.com/restaurant-page (AI will check marketing activity)" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
            </div>
            ...
            {results && (
                <div className="mt-8 border-t pt-6">
                    ...
                    <div className="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded-lg">
                        ...
                        <button onClick={() => copyToClipboard(results.emailDraft)} className="mt-3 bg-green-600 hover:bg-green-700 text-white text-sm font-semibold py-2 px-4 rounded-lg transition duration-200">
                            Copy Email Draft
                        </button>
                    </div>
                </div>
